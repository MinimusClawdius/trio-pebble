...
void face_classic_load(Window *window, Layer *root, GRect bounds) {
    APP_LOG(APP_LOG_LEVEL_INFO, "[CLASSIC] face_classic_load ENTER bounds=%dx%d", (int)bounds.size.w, (int)bounds.size.h);
    (void)window;
    int w = bounds.size.w;
    int h = bounds.size.h;
    ...
    int comp_y = dynamic_header_h + graph_h;
    APP_LOG(APP_LOG_LEVEL_INFO, "[CLASSIC] Creating s_comp_layer at y=%d (dynamic_header_h=%d, graph_h=%d, h=%d)",
            comp_y, dynamic_header_h, graph_h, h);
