...
- Complications bar improvements:
  ...
  - Added custom drawn heart icon next to Heart Rate (unicode ♥ didn't render reliably).
...
- All other settings hardcoded in the watchface.
- Version 2.18.3
...
### v2.17.8
- Offline config page is now the default (USE_OFFLINE_CONFIG = true).
- Full config HTML embedded via data: URL (no internet required).
- Version 2.17.8
...
- Made complication text height (text_h) dynamic based on header_size.
- Improved vertical centering for larger fonts (GOTHIC_24 when hs=2).
- Version 2.17.4
...
- Improved visibility into layout calculations for 200x228 screens.
